...
This produces images named `validibot-validator-backend-<slug>:latest` (slugs: `energyplus`, `fmu`, `shacl`, `schematron`). The worker finds each one by that name automatically — there's nothing to configure.
