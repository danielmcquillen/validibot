...
- **Container-engine socket access**: The worker mounts the host path selected
  by `VALIDATOR_CONTAINER_SOCKET` at `/var/run/docker.sock` to spawn advanced
  validators. The default rootful Docker socket grants root-equivalent host
  authority; rootless Docker reduces that authority. Treat the worker as a
  sensitive service in either mode.
