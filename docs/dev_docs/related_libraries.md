...
- `../vb_shared`: Shared Python package installed into this project as the `vb-shared` dependency. Source lives in the neighbouring repository, but a vendored version is located inside the virtual environment at `.venv/lib/python3.x/site-packages/vb_shared`. Any changes should be made in vb_shared directly and it's version incremented.
...
sv_modal installs vb_shared via a github reference. Do not change this. When vb_shared is updated, you must reinstall it i the sv_modal project like:

    uv lock --upgrade-package vb-shared && uv sync --dev.
...
- During development, open the relevant modules in `../vb_shared` and `../sv_modal` alongside the Django code to avoid contract drift.
...
- When `vb_shared` changes, bump its version, publish/push it, then upgrade both this project and `sv_modal` to that version (do **not** swap `sv_modal` to a local editable reference). In `sv_modal`, run `uv lock --upgrade-package vb-shared && uv sync --dev`.
...
- Use `sv_modal/update_modal.sh` as a one-stop script: it upgrades `vb_shared`, syncs deps, creates prod/test Modal volumes for FMI and EnergyPlus, and redeploys both Modal apps so Django integration tests hit current runners.
