...
- Use `sv_modal/update_modal.sh` as a one-stop script: it upgrades `validibot_shared`, syncs deps, creates prod/test Modal volumes for FMI and EnergyPlus, and redeploys both Modal apps so Django integration tests hit current runners.
