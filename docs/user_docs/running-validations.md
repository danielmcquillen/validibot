...
Document size limits, supported formats, and tips for preparing files before upload. Note any automatic metadata SimpleValidations captures from filenames.

## Launch Through the API
Summarize the REST endpoint for `POST /api/workflows/<id>/start/` and reference the detailed API sections for payload formats. Mention how role requirements (EXECUTOR) are checked before a run starts.
