...
For Docker Compose installations, also include:

- Validibot version
- Deployment environment (GCP, Docker Compose, etc.)
- Relevant log entries
- Output from `python manage.py check_validibot --verbose`

## Docker Compose: Health Check Command

Docker Compose administrators can use the built-in health check command to diagnose configuration issues:
...
- **Database**: Connection and PostgreSQL version
- **Migrations**: All migrations applied
- **Cache**: Redis/cache connectivity
- **Storage**: File storage read/write access
- **Site Configuration**: Django Sites framework
- **Roles & Permissions**: Required roles and permissions exist
- **Validators**: System validators configured
- **Background Tasks**: Celery broker connectivity
- **Docker**: Docker availability for advanced validators
- **Email**: SMTP server reachability
- **Security**: DEBUG mode, SECRET_KEY, ALLOWED_HOSTS
...
- As part of monitoring (use `--json` output)

### Support Channels

**Community Edition**: Open an issue on [GitHub](https://github.com/danielmcquillen/validibot/issues)
