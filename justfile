...
# Usage: just selfhosted <command>
# Examples:
#   just selfhosted deploy
#   just selfhosted backup-db
#   just selfhosted health-check
mod selfhosted 'just/selfhosted'
...
default:
    ...
    @echo "    just selfhosted <command> # Self-hosted Docker Compose"
    @echo ""
    @echo "Examples:"
    ...
    @echo "    just selfhosted deploy    # Deploy self-hosted"
    @echo ""
...
# Usage: just status gcp prod | just status selfhosted
[no-cd]
platform-status platform stage="":
    ...
    case "{{platform}}" in
        ...
        *)
            ...
            echo "Supported: gcp, aws, selfhosted"
            exit 1
            ;;
    esac
