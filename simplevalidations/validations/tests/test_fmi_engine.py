...
def test_fmi_engine_success_path():
    ...
    upload = _fake_fmu()
    validator = create_fmi_validator(org=org, project=workflow.project, name="Test FMU", upload=upload)
...
def test_fmi_engine_rejects_missing_fmu():
    org = OrganizationFactory()
    workflow = WorkflowFactory(
        org=org,
        allowed_file_types=[SubmissionFileType.BINARY],
