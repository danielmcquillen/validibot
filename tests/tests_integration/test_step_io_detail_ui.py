"""
Selenium-based integration tests for the step I/O detail UI.

These tests verify that:
1. Step I/O info buttons open modals correctly
2. Modal content displays step I/O information
3. Modals can be closed properly
4. The complete step I/O link navigates correctly
"""
...
@pytest.fixture
def validator_with_step_io(test_user_with_org):
    """Create a validator with input and output values."""
    ...
    return validator, input_definition, output_definition
...
class TestStepIODetailModal:
    """Integration tests for step I/O detail modals using Selenium."""

    def test_step_io_info_button_opens_modal(
        self,
        selenium_driver,
        live_server,
        test_user_with_org,
        validator_with_step_io,
    ):
        """Clicking an info button should open the step I/O detail modal."""
        user, org = test_user_with_org
        validator, input_definition, output_definition = validator_with_step_io
        ...
        # Wait for page to load and find the info button for step input
        info_button_selector = (
            f'button[data-bs-target="#modal-step-io-detail-{input_definition.id}"]'
        )
        ...
        modal_selector = f"#modal-step-io-detail-{input_definition.id}"
        ...
        # Verify modal contains step I/O information
        modal_body = modal.find_element(By.CSS_SELECTOR, ".modal-body")
        modal_text = modal_body.text

        assert input_definition.contract_key in modal_text
        assert "Floor Area (m2)" in modal_text or input_definition.label in modal_text
        assert "Input" in modal_text
        assert "Number" in modal_text

    def test_step_io_modal_can_be_closed(
        self,
        selenium_driver,
        live_server,
        test_user_with_org,
        ...
        """The step I/O detail modal should close normally."""
        user, org = test_user_with_org
        validator, input_definition, output_definition = validator_with_step_io
    ...
    def test_view_all_step_outputs_link_navigates_correctly(
        self,
        selenium_driver,
        live_server,
        test_user_with_org,
        ...
        """Test that 'View all' link navigates to step I/O list page."""
        user, org = test_user_with_org
        validator, input_definition, output_definition = validator_with_step_io
        ...
        # Wait for navigation to step I/O list page
        WebDriverWait(selenium_driver, 10).until(
            expected_conditions.url_contains("/step-io/"),
        )

        # Verify we're on the step I/O list page
        assert "/step-io/" in selenium_driver.current_url

        # Verify page content
        page_source = selenium_driver.page_source
        assert "Inputs and outputs for" in page_source
        assert validator.name in page_source
        assert input_definition.contract_key in page_source
        assert output_definition.contract_key in page_source

    def test_step_io_list_back_button_navigates_correctly(
        self,
        selenium_driver,
        live_server,
        test_user_with_org,
        ...
        """Test that back button on step I/O list navigates to validator detail."""
        user, org = test_user_with_org
        validator, input_definition, output_definition = validator_with_step_io
        ...
        # Navigate directly to step I/O list page
        step_io_path = reverse(
            "validations:validator_step_io_list",
            kwargs={"slug": validator.slug},
        )
        step_io_url = f"{live_server.url}{step_io_path}"
        selenium_driver.get(step_io_url)
        ...
        assert "/step-io/" not in selenium_driver.current_url
    ...
    def test_no_template_comments_in_page(
        ...
        assert "StepIODefinition instance" not in page_source, "Docstring leaked"
