...
that need to run in Validibot, regardless of the execution backend:
    ...
    - Celery Beat (self-hosted Docker Compose deployments)
...
Each task is defined once here with all metadata needed for any backend:
    ...
    - Celery task path (for self-hosted)
...
class Backend(StrEnum):
    ...
    CELERY = "celery"  # Self-hosted via Celery Beat
...
class ScheduledTaskDefinition:
    ...
    # Celery configuration (self-hosted)
...
SCHEDULED_TASKS: tuple[ScheduledTaskDefinition, ...] = (
    ...
    ScheduledTaskDefinition(
        ...
        schedule_cron="*/10 * * * *",  # Every 10 minutes
        schedule_interval_minutes=10,
        description="Remove orphaned Docker validator containers (self-hosted only)",
        backends=(Backend.CELERY,),  # Only for self-hosted deployments
    ),
)
