...
These are used in Docker Compose deployments where Celery + Redis handles
...
#   cleanup_orphaned_containers   - Every 10 minutes (Docker Compose only)
...
def cleanup_orphaned_containers(self) -> dict:
    ...
    This task only runs on Docker Compose deployments.
