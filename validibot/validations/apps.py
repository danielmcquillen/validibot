...
class ValidationsConfig(AppConfig):
    ...
    def _should_run_startup_cleanup(self) -> bool:
        ...
        Only runs in Dramatiq worker processes on self-hosted deployments.
        ...
        # Check if we're in a Dramatiq worker process
        # Dramatiq sets this environment variable when running workers
        if "dramatiq" not in os.environ.get("DJANGO_SETTINGS_MODULE", ""):
            # Alternative: check for dramatiq in sys.argv
            import sys

            if not any("dramatiq" in arg for arg in sys.argv):
                return False
