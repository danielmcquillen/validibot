    ...
    *,
    ...
    for contract_key in ("primary_model", "weather_file"):
        ...
        if port.envelope_channel == EnvelopeChannel.RESOURCE_FILES:
            ...
            try:
                _validate_input_file_item_matches_port(port=port, item=item)
            except ValueError as exc:
...
def _resolve_workflow_resource_port(
    *,
    ...
    _validate_artifact_port_cardinality(
        port=port,
        count=len(matches),
        source_description=(
    ...
    for item in matches:
        ...
        _validate_resource_file_item_matches_port(port=port, item=item)
    return matches
    ...
    *,
    ...
    if binding.source_scope == BindingSourceScope.UPSTREAM_ARTIFACT:
        try:
            ...
            _validate_file_uri_matches_port(port=port, uri=uri)
            _validate_artifact_ref_matches_port(port=port, artifact_ref=artifact_ref)
        except ValueError as exc:
            _record_and_raise_artifact_resolution_error(
                run=run,
                port=port,
...
def _validate_input_file_item_matches_port(*, port, item: InputFileItem) -> None:
    """Validate a backend input file item against its artifact-port contract."""

    _validate_artifact_port_role(
        port=port,
        observed_role=item.role,
        source_description=f"input file '{item.uri}'",
    )
    _validate_artifact_port_data_format(
        port=port,
        observed_data_format=_data_format_from_energyplus_uri(item.uri or item.name),
        source_description=f"input file '{item.uri}'",
    )
    _validate_artifact_port_media_type(
        port=port,
...
def _validate_resource_file_item_matches_port(*, port, item: ResourceFileItem) -> None:
    ...
    _validate_file_uri_matches_port(port=port, uri=item.uri)
    _validate_artifact_port_data_format(
        port=port,
    ...
    _validate_artifact_port_media_type(
        port=port,
        observed_media_type=(
            _media_type_from_energyplus_uri(item.uri)
...
def _validate_artifact_ref_matches_port(*, port, artifact_ref: dict) -> None:
    ...
    inferred_media_type = _media_type_from_energyplus_uri(uri)

    _validate_artifact_port_data_format(
        port=port,
        observed_data_format=data_format or _data_format_from_energyplus_uri(uri),
...
def _validate_artifact_port_role(
    *,
    port,
    observed_role: str,
...
def _validate_artifact_port_cardinality(
    *,
    port,
    count: int,
    source_description: str,
...
def _validate_artifact_port_data_format(
    *,
    port,
...
def _validate_artifact_port_media_type(
    *,
    port,
...
def _media_type_from_energyplus_uri(uri: str) -> str:
    """Infer an EnergyPlus MIME type from a filename or storage URI."""
...
def _data_format_from_energyplus_uri(uri: str) -> str:
    """Infer the artifact data format from an EnergyPlus filename or URI."""
