"""
Tests for the step input resolution engine.

The resolution engine is the bridge between ``StepInputBinding`` (which
declares *where* to find a step input) and the actual data extraction.
...
- Default value fallback for optional inputs
- Structured errors for missing required inputs
- Audit tracing via ``ResolvedInputTrace`` rows

The core function ``resolve_step_input()`` resolves a single binding;
``resolve_step_input_values()`` batch-resolves all bindings for a step.
"""
...
from validibot.validations.services.path_resolution import StepInputResolutionError
from validibot.validations.services.path_resolution import resolve_step_input
from validibot.validations.services.path_resolution import resolve_step_input_values
...
class ResolveStepInputTests(TestCase):
    """Tests for the single-input resolve_step_input() function."""

    def _make_binding(self, *, step=None, scope=None, path="", **kwargs):
        """Create a step I/O definition and binding for testing."""
    ...
    def test_required_input_missing_returns_error_result(self):
        """When a required input's path doesn't match anything in the
        submission and no default is configured, resolve_step_input
        ...
        result = resolve_step_input(binding, submission_data={"other": 1})
        self.assertFalse(result.resolved)
        self.assertIn(
            binding.io_definition.contract_key,
    ...
    def test_batch_raises_after_collecting_all_errors(self):
        ...
        ALL audit traces before raising StepInputResolutionError. This
        ensures operators get complete diagnostic information — which
        inputs resolved, which failed — not just the first failure.
        """
        ...
        # Two required step inputs, both missing from submission
        ...
        with pytest.raises(StepInputResolutionError) as exc_info:
        ...
        # Both missing inputs should be mentioned in the error message
        assert "input_a" in str(exc_info.value)
        assert "input_b" in str(exc_info.value)

        # Traces for BOTH inputs should be attached to the exception
        assert len(exc_info.value.traces) == 2  # noqa: PLR2004

    def test_optional_input_missing_returns_unresolved(self):
        """Optional inputs (is_required=False) with no matching path and
        no default should return a ResolvedStepInput with resolved=False and
    ...
    def test_default_value_used_when_path_not_found(self):
        ...
        result = resolve_step_input(binding, submission_data={})
        self.assertTrue(result.resolved)
    ...
    def test_submission_metadata_scope(self):
        ...
        not the payload — used for inputs sourced from submission
    ...
    def test_blank_path_falls_back_to_contract_key(self):
        ...
        look up the input's contract_key as a top-level key in the
    ...
    def test_upstream_step_output_resolution(self):
    ...
    def test_upstream_step_missing_output_returns_error(self):
    ...
    def test_metadata_flat_key_energyplus_style(self):
        ...
        path from ensure_step_input_bindings through resolution.
        """
        binding = self._make_binding(
            scope=BindingSourceScope.SUBMISSION_METADATA,
            path="floor_area_m2",
...
class ResolveStepInputValuesTests(TestCase):
    """Tests for the batch resolve_step_input_values() function."""
    ...
    def test_batch_creates_trace_instances(self):
        ...
        _, traces = resolve_step_input_values(
            step,
            step_run,
        ...
        self.assertEqual(trace.input_contract_key, "pressure")
    ...
    def test_batch_resolves_metadata_scoped_inputs(self):
        """The batch resolver should pass submission_metadata through to
        individual input resolution — critical for EnergyPlus inputs that
    ...
    def test_batch_resolves_upstream_step_outputs(self):
        """The batch resolver should correctly resolve inputs scoped to
        UPSTREAM_STEP, navigating the 'step_key.output_name' path against
        the upstream step-output dictionary.
        """
        step = WorkflowStepFactory()
        run = ValidationRunFactory(workflow=step.workflow)
        step_run = ValidationStepRunFactory(
...
# This bridges a workflow signal mapping to a step input: the mapping extracts
# a value from the submission, and a SIGNAL-scoped binding feeds it into the
# validator.


class ResolveSignalScopeTests(TestCase):
    """Tests for the SIGNAL scope in resolve_step_input().
...
class ResolveStepInputSignalScopeBatchTests(TestCase):
    """Tests for SIGNAL scope in the batch resolve_step_input_values().
    ...
    def test_batch_mixed_scopes(self):
        ...
        payload_input = StepIODefinitionFactory(
            workflow_step=step,
            validator=None,
            direction=StepIODirection.INPUT,
        ...
        StepInputBindingFactory(
            workflow_step=step,
            io_definition=payload_input,
            source_scope=BindingSourceScope.SUBMISSION_PAYLOAD,
            source_data_path="T_outdoor",
        )

        # Signal-scoped binding
        signal_sourced_input = StepIODefinitionFactory(
            workflow_step=step,
            validator=None,
            direction=StepIODirection.INPUT,
        ...
        StepInputBindingFactory(
            workflow_step=step,
            io_definition=signal_sourced_input,
            source_scope=BindingSourceScope.SIGNAL,
            source_data_path="solar_irradiance",
        )
