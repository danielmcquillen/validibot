...
def test_gcs_cloud_run_contract_is_unconditionally_attempt_scoped():
    """GCP has no reduced-isolation configuration or legacy credential mode."""
    report = get_storage_capability_report()
    ...
    assert report.immutable_reads is True
    assert report.attempt_scoped_authority is True
    assert "Policy Troubleshooter" in report.limitations[-1]
