"""
Tests for the SimpleValidatorEngine base class.

Uses a concrete stub subclass (_StubSimpleEngine) that implements
...
class _StubSimpleEngine(SimpleValidatorEngine):
    """
...
class SimpleValidatorEngineLifecycleTests(TestCase):
    """Tests the template method validate() lifecycle on SimpleValidatorEngine."""
    ...
    def test_happy_path_passes_with_no_issues(self):
        ...
        engine = _StubSimpleEngine(parsed_value={"ok": True})
    ...
    def test_file_type_rejection_returns_early(self):
        ...
        engine = _StubSimpleEngine(file_type_issue=issue)
    ...
    def test_empty_signals_result_in_none(self):
        ...
        engine = _StubSimpleEngine(parsed_value={})

        result = engine.validate(self.validator, self.submission, self.ruleset)

        self.assertIsNone(result.signals)

    def test_run_context_stored_on_engine(self):
        """The run_context argument is stored on the engine instance."""
        engine = _StubSimpleEngine(parsed_value={})
...
class SimpleValidatorEngineAssertionTests(TestCase):
    """Tests assertion evaluation within the SimpleValidatorEngine lifecycle."""
