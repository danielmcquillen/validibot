...
class TestResolveInputValue:
    ...
    def test_top_level_slug(self):
        ...
        assert resolve_input_value(payload, binding_path="", slug="temperature") == 21.3  # noqa: PLR2004

    def test_dotted_binding_path(self):
        payload = {"building": {"metadata": {"area": 500.0}}}
        result = resolve_input_value(
            payload,
            binding_path="building.metadata.area",
            slug="area",
        )
        assert result == 500.0  # noqa: PLR2004

    def test_binding_path_overrides_slug(self):
        """When binding_path is set, slug is ignored."""
        payload = {"area": 999, "geometry": {"floor_area": 500.0}}
        result = resolve_input_value(
            payload,
            binding_path="geometry.floor_area",
            slug="area",
        )
        assert result == 500.0  # noqa: PLR2004

    def test_none_binding_path_falls_back(self):
        payload = {"width": 3.5}
        result = resolve_input_value(payload, binding_path=None, slug="width")
        assert result == 3.5  # noqa: PLR2004

    def test_whitespace_binding_path_falls_back(self):
        payload = {"width": 3.5}
        result = resolve_input_value(payload, binding_path="  ", slug="width")
        assert result == 3.5  # noqa: PLR2004

    def test_missing_path_returns_none(self):
        payload = {"temperature": 21}
        result = resolve_input_value(payload, binding_path="no.such.path", slug="x")
        assert result is None

    def test_non_dict_payload(self):
        assert resolve_input_value("not a dict", binding_path="x", slug="x") is None
        assert resolve_input_value(None, binding_path="x", slug="x") is None
        assert resolve_input_value(42, binding_path="x", slug="x") is None

    def test_deeply_nested(self):
        payload = {"a": {"b": {"c": {"d": {"value": 7}}}}}
        result = resolve_input_value(payload, binding_path="a.b.c.d.value", slug="x")
        assert result == 7  # noqa: PLR2004

    def test_null_intermediate(self):
        payload = {"a": None}
        assert resolve_input_value(payload, binding_path="a.b", slug="x") is None

    def test_intermediate_value_is_list(self):
        """resolve_input_value only supports dict traversal, not arrays."""
        payload = {"items": [{"id": 1}]}
        assert resolve_input_value(payload, binding_path="items.0", slug="x") is None
