...
class TestBillingAttribution:
    """Tests for billing attribution for workflow guests."""

    def test_guest_usage_billed_to_workflow_org(self):
        """Test that guest usage is billed to the workflow owner's org."""
        # Create orgs
        workflow_owner_org = OrganizationFactory(name="Workflow Owner")

        # Create a guest user with no active memberships
        guest = UserFactory(orgs=[])
        Membership.objects.filter(user=guest).delete()

        # Give the guest a grant to a workflow in workflow_owner_org
        workflow = WorkflowFactory(org=workflow_owner_org, is_active=True)
        WorkflowAccessGrant.objects.create(
            workflow=workflow,
            user=guest,
            is_active=True,
        ...
        # Verify the user is identified as guest
        assert guest.is_workflow_guest is True

        # The billing org should be the workflow's org, not the guest's
        # This is tested by ensuring the method checks the right org
        billing_org = workflow_owner_org
        if guest.is_workflow_guest:
            billing_org = workflow.org

        assert billing_org == workflow_owner_org

    def test_member_usage_billed_to_active_org(self):
        """Test that org member usage is billed to their active org."""
        # Create an org member
        org = OrganizationFactory(name="Member Org")
        member = UserFactory(orgs=[])
        Membership.objects.filter(user=member).delete()
        grant_role(member, org, RoleCode.EXECUTOR)

        # Create a workflow - used to verify the billing context
        WorkflowFactory(org=org, is_active=True)

        # Member should not be a guest
        assert member.is_workflow_guest is False

        # For members, billing goes to the org they're working in
        billing_org = org  # The standard behavior

        assert billing_org == org

    def test_guest_with_grant_to_multiple_workflows(self):
        """Test guest with grants to workflows in different orgs."""
        org1 = OrganizationFactory(name="Org One")
        org2 = OrganizationFactory(name="Org Two")
